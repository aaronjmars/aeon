...
export const BUILTIN_SECRETS: Omit<Secret, 'isSet'>[] = [
  ...
  { name: 'HOOK_MAINNET_OK', group: 'Skill Keys', description: 'Mainnet safety lock for deploy-uni-hook - set to 1 to allow real-money mainnet broadcasts (on top of the always-required arm: + explicit chain:). Leave unset to keep deploys testnet-only. A third, operator-level lock so a stray armed message can never reach mainnet on an instance that did not opt in.' },
